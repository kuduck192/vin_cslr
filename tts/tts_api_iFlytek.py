...
import json, sys, base64, hmac, hashlib, ssl, logging, wave
...
def call_iflytek_api(txt: str) -> bytes:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python tts_api.py <input.json>")
        sys.exit(1)
    ...
    audio_bytes = call_iflytek_api(text)
    ...
    logger.info("[TTS] Saved output.wav")
